                ...
                proyectos.map((proyecto) => (
                    <article key={proyecto.id} className="w-full flex items-center justify-center flex-col md:justify-between md:flex-row">
                        ...
                        <div className="flex justify-center flex-col items-center gap-5 text-white text-lg md:ml-10 xl:ml-0">
                            <Link to={proyecto.repo} target="_blank" className="bg-blue-700 rounded-xl p-2 xl:w-[10rem] xl:text-center">Repositorio</Link>
                            {
                                proyecto.tipo === 'Demo' ? (
                                    <Link to={proyecto.Demo} target="_blank" className="bg-slate-500 rounded-xl p-2 xl:w-[5rem] xl:text-center">Demo</Link>
                                ): (
                                    <Link to={`/Videos/${proyecto.id}`} className="bg-blue-400 rounded-xl p-2 xl:w-[5rem] xl:text-center">Video</Link>
                                )
                            }
                        </div>
                    </article>
